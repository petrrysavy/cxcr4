...
UNICLUST=./data/uniref30_2023/UniRef30_2023_02
...
do
  ...
  if [ -f "$MSADIR/$ID.a3m" ]; then
    ...
    $HHBLITS -i $FASTA -d $UNICLUST -E 0.001 -all -oa3m $MSADIR/$ID'.a3m' # -maxseq 250000 # -maxfilt did not have a positive impact, however, -maxseq is only output, right?
  fi
done
...
echo Predicting...
~/.conda/envs/it/bin/python3 ./src/run_alphafold_single.py --complex_id $COMPLEX_ID \
--msa1 $MSA1 \
--msa2 $MSA2 \
--data_dir $DATADIR \
